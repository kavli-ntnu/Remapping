...
for iExp = 1:3
    ...
    for iMod = 1:numMods
        ...
        if isempty(gStats.spacing) 
            ...
            if iExp == 2
                hLength = nanmean(gStats.spacing(1:2))*tan(pi/6); % third value calculated is incorrect, remove it
...
%         axis off
    ...
    for iPair = 1:size(finalModPts,2)
        currentPair = finalModPts{1,iPair};
        plot([currentPair(1,1) currentPair(2,1)],[currentPair(1,2) currentPair(2,2)],'k-')
    end

    theoreticalMax(1,1) = spacingStore(1,2) * tan(pi/6);
    theoreticalMax(1,2:3) = spacingStore(1,3) * tan(pi/6);
   
    normalizedDist = distStore(iExp,:) ./ theoreticalMax
